...
/*
Notes from YT overview playlist:
...
public:
  ...
  Iterator find(const T &query) const {
    auto it  = Iterator(root, find_impl(root, query, less), less);
    return it;
  }
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    if (empty_impl(node)) // node has no children
      return nullptr;
    else if (!less(query, node->datum) && !less(node->datum, query)) // query and node datum are equal
      return node;
    else if (less(query, node->datum)) // query is less than node datum
      return find_impl(node->left, query, less);
    else // node datum is less than query
  ...
  static Node * min_element_impl(Node *node) {
    if (empty_impl(node))
      return nullptr;
    else if (empty_impl(node->left)) //seg fault here, why?
      return node;
    else  
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    if (empty_impl(node)) // checks for empty tree
      return true;
    else if (!empty_impl(node->left) && less(node->datum, (node->left)->datum)) // ensures each node has a lesser left child
      return false;
    else if (!empty_impl(node->right) && less((node->right)->datum, node->datum)) // ensures each node has a greater right child
      return false;
    else if (!empty_impl(node->left) && less(node->datum, max_element_impl(node->left)->datum)) // ensures all elements to the left of each node are lesser than it
      return false;
    else if (!empty_impl(node->right) && less(min_element_impl(node->right)->datum, node->datum)) // ensures all elements to the right of each node are greater than it
      return false;
    else
    {
  ...
  //make sure you're checking all cases
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) { // this function is causing the seg fault in the compile check
    if (empty_impl(node))
      return nullptr;
    else if (!less(val, node->datum)) // node datum is less than or equal to val
      return min_greater_than_impl(node->right, val, less);
    else if ((less(val, node->datum) && (empty_impl(node->left))) || !less(val, node->left->datum))
      return node;
    else if (less(val, node->datum)) // val is less than node datum
